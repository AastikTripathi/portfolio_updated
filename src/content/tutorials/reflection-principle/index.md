...
1. **Modular Residue Invariants**: Quantities that remain invariant modulo $m$. (e.g. Modulo $3$ invariants in chameleon color-changing problems).
1. **Grid & Coloring Invariants**: Assigning colors or algebraic values ($1, -1, i$) to grid cells to show certain tilings or configurations are impossible.
1. **Monovariants**: Quantities that strictly increase or decrease with each step, proving that processes must terminate or cannot loop.
...
1. **Define**: Define a mathematical function $I(S)$ on the state (e.g. sum, product, parity, count modulo $k$).
1. **Verify**: Verify that $I(S_{\text{after}}) = I(S_{\text{before}})$ for every allowed move.
1. **Conclude**: Compare $I(S_{\text{initial}})$ with $I(S_{\text{target}})$. If they differ, the target state is impossible.
